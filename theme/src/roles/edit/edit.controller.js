...
angular.module('biffyApp')
    .controller('RolesEditController', function($q, $scope, $stateParams, $state, RestangularAppService, NotifierService) {
        ...
        if($scope.isAdd()) {
            ...
            RestangularAppService.one('roles', $scope.id).get().then(
                function(data) {
                    $scope.mode = 'Edit';
                    $scope.data = data;
                },
        ...
        $scope.store = function() {
            RestangularAppService.all('roles').post($scope.data).then(function() {
