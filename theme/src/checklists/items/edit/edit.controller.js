...
angular.module('biffyApp').controller(
    ...
    function($q, $scope, $stateParams, $state, RestangularAppService, NotifierService)
    {
        ...
        {
            RestangularAppService.one('checklistitems', $scope.id).get().then(
                function(data)
                {
                    $scope.mode = 'Edit';
                    $scope.data = data;
        ...
        {
            RestangularAppService.all('checklistitems').post($scope.data).then(
                function()
                {
                    $state.transitionTo('checklists.items');
                },
                function(data)
                {
