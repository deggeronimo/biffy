...
angular.module('biffyApp').controller(
    'ChecklistItemsDeviceTypesController',
    function($rootScope, $state, $scope, $q, $location, $filter, ngTableParams, RestangularAppService, NotifierService)
    {
        ...
        $scope.filter = function(data)
        {
            ...
            for (var i=0; i<data.length; i++)
            {
                ...
                else if (dataRow.checklist_item.name.toLowerCase().indexOf($scope.search.toLowerCase()) != -1)
                {
                    results.push(dataRow);
                }
            }
        ...
        $scope.tableParams = new ngTableParams(
            angular.extend(
                ...
                getData: function($defer, params)
                {
                    ...
                    RestangularAppService.all('deviceitemchecklists?full=1').getList(params.url()).then(
                        function(result)
                        {
