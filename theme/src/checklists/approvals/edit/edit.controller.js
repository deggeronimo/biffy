...
angular.module('biffyApp').controller(
    'ChecklistApprovalsEditController',
    function($q, $scope, $stateParams, $state, RestangularAppService, NotifierService)
    {
        ...
        {
            RestangularAppService.one('deviceapprovals', $scope.id).get().then(
                function(data)
                {
                    $scope.mode = 'Edit';
                    $scope.data = data;
        ...
        {
            RestangularAppService.all('deviceapprovals').post($scope.data).then(
                function()
                {
                    $state.transitionTo('checklists.approvals');
                },
                function(data)
                {
